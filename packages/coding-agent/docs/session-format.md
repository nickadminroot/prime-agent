...
Created when context is compacted. Stores a short notice plus an optional internal deterministic transcript of earlier messages.

```json
{"type":"compaction","id":"f6g7h8i9","parentId":"e5f6g7h8","timestamp":"2024-12-03T14:10:00.000Z","summary":"Previous reasoning and large tool outputs were shortened to compact the context.","firstKeptEntryId":"c3d4e5f6","tokensBefore":50000,"compactedMessages":[...]}
```

Optional fields:
- `details`: Implementation-specific data (e.g., `{ readFiles: string[], modifiedFiles: string[] }` for default, or custom data for extensions)
- `compactedMessages`: Internal provider-safe transcript messages. This optional field is persisted in JSONL and omitted from public connection/wire entries.
- `firstKeptEntryId`: `null` means the boundary is after the final pre-compaction entry; no retained prefix is replayed. Legacy entries always use a string ID.
...
3. If a `CompactionEntry` is on the path:
   - Emits internal `compactedMessages`, then messages from `firstKeptEntryId` to compaction
   - Emits the short notice
   - Then messages after compaction
   - Legacy entries without `compactedMessages` use their summary plus retained messages
...
- `appendCompaction(summary, firstKeptEntryId, tokensBefore, details?, fromHook?, customInstructions?, compactedMessages?)` - Add compaction
