...
export class AgentSession {
	...
	private async _createInlineRlmSubagentRuntime(
		...
		// discoverable immediately after runRlmChild returns (upstream contract).
		...
		// Force the child session file (header + pre-model entries) to disk before
		// the first await, so the child is discoverable right after runRlmChild
		// returns (upstream contract). The no-assistant guard in _persist would
