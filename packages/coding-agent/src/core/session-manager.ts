...
export interface CompactionEntry<T = unknown> extends SessionEntryBase {
	...
	details?: T;
	/** Provider-safe transcript retained by the deterministic default compaction. */
...
/** Remove internal deterministic transcript payloads before a connection/wire response. */
export function stripInternalCompactionEntry(entry: SessionEntry): SessionEntry {
	if (entry.type !== "compaction") return entry;
	const { compactedMessages: _compactedMessages, ...publicEntry } = entry;
	if (entry.firstKeptEntryId === null) {
		// Connection/tree consumers retain the historical string field shape;
		// the nullable boundary is an internal JSONL/build-context detail.
		return { ...publicEntry, firstKeptEntryId: "" } as SessionEntry;
	}
	return publicEntry as SessionEntry;
}

export function stripInternalCompactionTree(nodes: SessionTreeNode[]): SessionTreeNode[] {
	return nodes.map((node) => ({
		...node,
		entry: stripInternalCompactionEntry(node.entry),
		children: stripInternalCompactionTree(node.children),
	}));
}
...
export function buildSessionContext(
	...
	// When there's a compaction, model context contains the compacted transcript,
	// retained messages, then the notice; ordering metadata lets clients render it consistently.
	...
	if (compaction) {
		...
		// retainedMessageCount records the boundary after compacted and retained
		// messages so clients can place the notice without splitting the transcript.
		...
		messages.push(
			...(Array.isArray(compaction.compactedMessages) ? compaction.compactedMessages : []),
			...retainedMessages,
			createCompactionSummaryMessage(
				compaction.summary,
				...
				(Array.isArray(compaction.compactedMessages) ? compaction.compactedMessages.length : 0) +
